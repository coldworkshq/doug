...
// grouping.ts imports ./runs-time extensionless (web convention — see its own
// header). Node's ESM resolver does not guess extensions, so the loader has to
// be registered BEFORE the module is linked: a static import would be resolved
// at link time, before this line ever runs. Hence register + dynamic import.
register("./node-next-loader.mjs", import.meta.url);
const { groupRunsByPr, runCountLabel } = await import("./grouping.ts");
