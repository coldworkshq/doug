...
export function RunForensicsUnavailable({ error }: { error: string }): React.ReactNode {
  ...
  const unknownId = /→ HTTP 404$/.test(error);
  return (
...
export function RunForensics({ run, clearHref }: { run: RunDetail; clearHref: string }): React.ReactNode {
      ...
      <header className="flex items-start gap-5 border-b border-border py-5">
        <div className="min-w-0 flex-1">
          <div className="mono flex items-center gap-[7px] text-xs text-muted-foreground">
            <Link href={clearHref} className="text-foreground hover:text-[var(--iridescent)]">← runs</Link>
            <span>·</span>
            <span>{run.repo}</span>
            <span>·</span>
            {run.pr?.url ? (
