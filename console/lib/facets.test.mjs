...
import { FACET_KEYS, buildFacets, filterRuns, parseFacetSelection, serializeFacets } from "./facets.ts";
...
test("values within one facet are OR'd, values across facets are AND'd", () => {
  ...
  assert.deepEqual(
    filterRuns(runs, { band: ["flagged", "cleared"] }).map((r) => r.verdict_id),
    [1, 2, 3],
  );
  assert.deepEqual(
    filterRuns(runs, { band: ["flagged"], outcome: ["pending"] }).map((r) => r.verdict_id),
    [1],
  );
});
...
test("an empty selection matches everything rather than nothing", () => {
  ...
  assert.equal(filterRuns(runs, { band: [] }).length, 2);
});
...
test("an unknown value in the URL matches nothing and does not crash", () => {
  ...
  assert.deepEqual(filterRuns(runs, { band: ["purple"] }), []);
});
