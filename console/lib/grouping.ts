...
// Explicit `.ts`: this is a runtime import, not a type-only one, and
// `node --test --experimental-strip-types` resolves through Node's ESM
// resolver, which does not guess extensions. Type-only imports (above) are
// erased before that ever matters. tsconfig sets allowImportingTsExtensions
// to permit the suffix without requiring it elsewhere.
