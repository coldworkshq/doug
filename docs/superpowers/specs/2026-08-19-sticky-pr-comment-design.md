...
- **Two drainers, two jobs, one PR** — the `seq` guard prevents an older verdict overwriting a newer one only on the DISCOVERY path (a comment found by listing and matched by marker + authorship); a write through an already-stored `comment_id` — the common case once a PR has a comment — updates unconditionally, with no `seq` comparison. The claim row prevents two creates. Residual: `SKIP LOCKED` + two instances is the deployed configuration, so an older job racing a newer one is ordinary, not exotic; closing the stored-id path fully needs a `last_seq` column on `pr_comments` (issue #142). The worker's stale-head supersede already makes this rare in practice — a fresh job that sees a newer head has already superseded the older one before either reaches `upsert` — so the residual is one stale comment that self-heals on the next push.
