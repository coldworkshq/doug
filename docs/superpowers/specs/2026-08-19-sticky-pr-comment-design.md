...
- **D3 — On by default, opt-out per repository; Doug never deletes a comment.** `installation_repos.pr_comment BOOLEAN NOT NULL DEFAULT TRUE`, toggled beside the flag line. "On" means *for repos the tenant can see and toggle* — the worker posts only when an **active** `installation_repos` row exists with `pr_comment = TRUE` (D6). Turning it off stops updates; the last comment stays. Rejected: off by default forever; split by band.
  ...
  - **D3b — judgment call for Andrew (recommended: keep "never deletes", open a standing issue):** opt-out is forward-only; already-posted comments stay and removal is manual, per PR. `pull_requests: write` *does* allow deletion, so "delete Doug's comments on opt-out" is buildable; it is not built here because deleting under a bot identity on a tenant's PRs is its own surprise, and because the record of what Doug said is the thing the receipt exists to preserve. Recorded as GitHub issue #141 per the standing-issues rule, and priced in ADR-0014's Consequences as the persist half of ADR-0003's objection.
...
- **D8 — Permission denial is visible on the dashboard.** `installations.pr_comment_denied_at TIMESTAMPTZ NULL` set when `upsert` returns `denied`, cleared on the next `created|updated`; one banner on the Repositories view. **Shipped wording** (pinned by a contract test, superseding the sentence this spec originally proposed): *"PR comments are not posting. Doug's last attempt was refused (403) at {date}. The usual cause is the pull-requests write permission not being re-accepted in GitHub; a locked conversation, an archived repository, or secondary rate limiting produce the same code."* — three causes plus the timestamp of the last refusal, not two: `denied:403` is GitHub's response for all of them alike, so the banner hedges exactly as far as the evidence does rather than naming one cause with false confidence. Rejected: shipping a toggle that reads "on" while nothing ever posts and the only trace is a stderr line in a project with no alerting.
...
- Standing issues opened at implementation time (superseding the plan-time list above, which named a different pair): delete-on-opt-out, D3b (#141); the `seq` guard covers only the discovery path, not a write through a stored `comment_id` (#142 — since closed; §4's entry describes the shipped guard); worker comment-log fidelity — `failed:internal` can report a comment that actually landed, and the worker's log line collides with `pr_comment._log`'s prefix so failure tokens double-count on grep (#143). The partial-write race across the PATCH endpoint's two single-field store calls was filed separately, before this task, as #140.
...
- **Two drainers, two jobs, one PR** — the `seq` guard covers all three of `upsert`'s write sites (closed by issue #142; it originally covered only the DISCOVERY path, leaving a write through an already-stored `comment_id` — the common case once a PR has a comment — to update unconditionally). A discovered comment is compared against the `seq` in its own marker; a comment reached through a stored `comment_id` is compared against `pr_comments.last_seq`, a high-water mark that `store.claim_pr_comment_seq` tests and advances in ONE conditional UPDATE taken before the GitHub write, so the reservation is the comparison rather than a read with the round trip left open behind it. `last_seq` is NULL for a claim that has not created and for every row predating the column, and NULL never blocks a write. The claim row prevents two creates. `SKIP LOCKED` + two instances is the deployed configuration, so an older job racing a newer one is ordinary, not exotic — and the worker's stale-head supersede makes it rarer still, since a fresh job that sees a newer head has already superseded the older one before either reaches `upsert`.
