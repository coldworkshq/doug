...
**Note on the fixture.** `receipt-fixture.json` is a test-and-development artifact. **Amended 2026-08-12:** "every state at once" is unachievable — `preregistration.in_force: false` and a pending window's "will govern this window" row are mutually exclusive in a single payload. Ship the main fixture below plus a second minimal one, `receipt-fixture-no-prereg.json`, carrying `in_force: false`. And the fixtures must be *referenced by a test* (`lib/receipt-fixture.test.mjs`) asserting `isReceiptResponse` and the per-state outputs — an unreferenced fixture goes silently stale the first time a field is renamed, which defeats the point of having one. The main fixture exercises: a PR with two merges, one publication-governing and one not; one merge with a null `governing_verdict`; a 14-day window adjudicated `censored` with its own `prereg_hash`; a 60-day window `pending` with none; a verdict with `read.recorded: false` and `prompt_hash: null`; and `merged_head_sha: null` on the older merge. Unlike the scoreboard fixture, it is **not** wired as a runtime fallback — a receipt that cannot be loaded renders an honest error state, never invented evidence.
