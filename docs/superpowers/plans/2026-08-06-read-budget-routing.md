...
**Interfaces:**
- Consumes: existing `LOCKFILES` set (`features.py:13`), `PurePosixPath` (already imported)
...
def test_prose_covers_docs_and_lockfiles_but_not_their_manifests():
    ...
    Markdown cannot contain any of them, and a lockfile is generated bytes
    no human reads in review — so both rank last for the read budget.

    A manifest is the opposite: package.json and pyproject.toml carry real
    dependency decisions, which is exactly the kind of change that turns
    into a defect. They stay code."""
